...
export default function Minting() {
  ...
  async function connectTableland() {
    if (account && ethereumProvider) {
      ...
      await writeQuery(ethereumProvider, TABLE_NAME, {
        id: query.rows.length,
        ca: CONTRACT_ADDRESS,
        metadata: METADATA_URI,
        address: account
      })
      setSuccessMsg('Get successful for Participation..!')
    }
  }
  ...
  async function fetchTotalSupply() {
    ...
    setTotalSupply((await contract.tokenCounter()).toString());
  }
  ...
  return (
    <>
      <Image src={METADATA_IMG} width={300} height={300} layout="responsive" alt="image" />

      <h2 className="text-4xl mb-4">For Minting</h2>
      ...
      <div className="bg-gray-800 border border-t-red-300 border-r-blue-300 border-b-green-300 border-l-yellow-300 rounded p-8 space-y-4">
        ...
        <div>
          {/* <button type="button" onClick={connectTableland}>Connect Tableland</button> */}
          {/* <button type="button" onClick={connectTableland}>Add Whitelist</button> */}
        </div>
